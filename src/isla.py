...
def generar_mapa() -> list:
    ...
    vacia_x, vacia_y = random.randint(0, DIMENSIONES - 1), random.randint(0, DIMENSIONES - 1)
    mapa[vacia_x][vacia_y] = CELDA_VACIA
...
def genera_pista():
    ...
    if mapa[posicion_jugador[FILAS]] == CELDA_TESORO:
        posicion_tesoro, posicion = genera_pista_columnas()
        genera_pista_columnas(posicion_tesoro, posicion)
    elif mapa[posicion_jugador[COLUMNAS]] == CELDA_TESORO:
        posicion_tesoro, posicion = genera_pista_filas()
        genera_pista_filas(posicion_tesoro, posicion)
    else:
